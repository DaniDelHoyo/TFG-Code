...
Script that compares two DDI score files (trained files).
Input: python3 compare_traineds.py <dyer_trained> <DMDI_database> [outname]
'''
    ...
    with open(int_file) as filex:
        for line in filex:
            ...
            if len(line)==2:
                descr=line[0]
                descr=descr.strip()
                int_dic[descr]=line[1]
            else:
                pair=line[:2]
                pair.sort()
                descr=pair[0]+'|'+pair[1]
                descr=descr.strip()
                int_dic[descr]=line[2]
    return int_dic
